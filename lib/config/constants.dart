import 'package:flutter/material.dart';

// Colors
const blackColor = Colors.black;
const blackClr45 = Colors.black45;
const whiteColor = Colors.white;
const whiteClr24 = Colors.white24;
const greyColor = Colors.grey;
const blueColor = Colors.blue;
final blueClr300 = Colors.blue[300];
final blueClr100 = Colors.blue[100];
final greyClr300 = Colors.grey[800];
final greyClr600 = Colors.grey[600];

// TextStyles
const whiteStyle = TextStyle(color: Colors.white);
const blackStyle = TextStyle(color: Colors.black);
const whiteTxt18 = TextStyle(color: Colors.white, fontSize: 18);
const whiteTxt20 = TextStyle(color: Colors.white, fontSize: 20);
const whiteTxt22 = TextStyle(color: Colors.white, fontSize: 22);
const white38Txt14 = TextStyle(color: Colors.white38);
final blackTxt50 = TextStyle(color: Colors.black, fontSize: 50,fontWeight: FontWeight.bold);
final blackTxt25 = TextStyle(color: Colors.black, fontSize: 25,fontWeight: FontWeight.bold);
final blackTxt15 = TextStyle(color: Colors.black, fontSize: 15,fontWeight: FontWeight.bold);



final blackTxt150 = TextStyle(color: Colors.black, fontSize: 150,fontWeight: FontWeight.bold);
const splashTxtStyle = TextStyle(
  color: whiteColor,
  fontSize: 40,
);

// Height and Width
const sbHeight10 = SizedBox(height: 10);
const sbHeight20 = SizedBox(height: 20);
const sbHeight30 = SizedBox(height: 30);
const sbWidth10 = SizedBox(width: 10);
const sbWidth20 = SizedBox(width: 20);
const sbWidth30 = SizedBox(width: 30);
const sbWidth40 = SizedBox(width: 40);


